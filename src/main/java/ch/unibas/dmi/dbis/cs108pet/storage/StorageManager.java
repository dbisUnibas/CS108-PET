...
/**
 * The {@link StorageManager} manages {@link SaveFile} for a cs108-pet session.
 * <p>
 * For this purpose, the StorageManager is able to open any pet related file and may open related files as well.
 ...
 * <p>
 * It keeps track of all opened files and is enables other components of pet to read and write from the files.
 * <p>
 * Furthermore it provides the means for the UI to list and show pet related files.
 *
 * @author loris.sauter
 */
public class StorageManager {
   ...
   *
   * @param dir The folder in which cs108pet should store all files
   */
   ...
   * @return
   */
  public List<CS108PETFile> listFiles() {
    ArrayList<CS108PETFile> files = new ArrayList<>();
    Arrays.stream(dir.listFiles(CS108PET_FILE_FILTER)).forEach(f -> {
      LOGGER.debug("Processing file {}", f);
      try {
        CS108PETFile.Type t = CS108PETFile.Type.valueOf(FileUtils.getFileExtension(f).toUpperCase());
        CS108PETFile rf = new CS108PETFile(f, t);
        files.add(rf);
  ...
  private static List<String> getKnownExtensions() {
    ...
    for (CS108PETFile.Type type : CS108PETFile.Type.values()) {
  ...
  public static final FileFilter CS108PET_FILE_FILTER = pathname -> getKnownExtensions().contains(FileUtils.getFileExtension(pathname));
